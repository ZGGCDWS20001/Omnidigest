...
class BreakingAlerter:
    ...
    def check_and_push(self):
        ...
        """
        try:
            pushable_stories = self.db.get_pushable_stories(threshold=self.threshold)
            ...
            for story in pushable_stories:
                ...
                self._push_story_alert(story, is_update=is_update)
                self.notified_stories.add(cache_key)
                # Mark persistent state in DB so restart doesn't resend
                self.db.mark_story_pushed(story_id, peak_score)
                    
        except Exception as e:
            logger.error(f"Error checking breaking story alerts: {e}")

    def _push_story_alert(self, story: dict, is_update: bool = False):
        """
        ...
        story_events = self.db.get_story_events(story['id'])
